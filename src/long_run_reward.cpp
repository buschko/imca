...
*
* Source description: 
*	 Compute long-run reward for an MA
*/
...
*/
static void set_obj_function_lrr(LP& lp_model, SparseMatrix *ma, bool max, vector<bool> mec, bool *locks) {
	//bool *goals = ma->goals;
...
* @param max identifier for maximum/minimum
*/
static void set_obj_function_ssp_lrr(LP& lp_model, SparseMatrix *ma, SparseMatrixMEC *mecs, bool max,
...
* @param max identifier for maximum/minimum
*/
static void set_constraints_lrr(LP& lp_model, SparseMatrix *ma, bool max, vector<bool> mec, bool *locks) {
	unsigned long i;
		...
		if(mec[state_nr]){// && !locks[state_nr]) {
			...
			for (choice_nr = state_start; choice_nr < state_end; choice_nr++) {
				...
				if(!bad) {
					...
					exit_rate=0;
					// get reward
					reward = (-1)*rewards[choice_nr];
					unsigned long r_start = rate_starts[state_nr];
					unsigned long r_end = rate_starts[state_nr + 1];
					for (unsigned long j = r_start; j < r_end; j++) {
						exit_rate = exit_rates[j];
					}
					if(exit_rate > 0 && reward < 0){
						reward /= exit_rate;
					} else if(exit_rate > 0 && reward != 0) {
						reward = -1/exit_rate;
					}
					if (reward < 0 && exit_rate == 0){
						//row.add(ma->n,reward);
                    } else if(exit_rate != 0){
                        row.setCol(ma->n,(-1/exit_rate));
                    }
					// write condition
					if(goals[state_nr]) {
						row.setValue(reward);
 ...
 * @param max identifier for maximum/minimum
 */
static void set_constraints_lrr_ratio(LP& lp_model, SparseMatrix *ma, bool max, vector<bool> mec, bool *locks) {
	unsigned long i;
	unsigned long state_nr;
	unsigned long choice_nr;
	unsigned long states = ma->n + 1;
...
string getEnvVarLrr( std::string const & key )
{
...
Real compute_stochastic_shortest_path_problem_lrr(SparseMatrix *ma, SparseMatrixMEC *mecs, vector<Real> lra_mec, bool max) {
	...
	set_obj_function_ssp_lrr(lp_model,ma,mecs,max,mec,lra,locks,ssp_nr);
	dbg_printf("make constraints\n");
	set_constraints_ssp_lrr(lp_model,ma,mecs,max,mec,locks,lra,ssp_nr,mecNr,lra_mec);
...
Real get_mec_reward(SparseMatrix *ma, vector<bool> mec, LP& lp_model) {
	Real amount=0;
    Real lrr = lp_model.getObjective();
    ...
    for (state_nr = 0; state_nr < ma->n; state_nr++) {
		if(mec[state_nr]){// && !locks[state_nr]) {
			...
			for (choice_nr = state_start; choice_nr < state_end; choice_nr++) {
				...
				for (i = i_start; i < i_end; i++) {
					if(mec[cols[i]] && !bad) {
						check += non_zeros[i] * lp_model.getPrimal(cols[i]);
					}
					else{
						bad=true;
					}
				...
				if(!bad) {
					...
					if(check == lp_model.getPrimal(state_nr)) {
...
Real compute_long_run_reward(SparseMatrix *ma, bool max) {
	...
	for(unsigned long mec_nr=0; mec_nr < mecs->n; mec_nr++) {
        ...
        reward_mec[mec_nr]=lp_model.getObjective();
...
*/
void compute_markovian_vector_lrr(SparseMatrix* ma, vector<Real>& v, const vector<Real> &u, bool* locks){
	unsigned long *row_starts = (unsigned long *) ma->row_counts;
	unsigned long *choice_starts = (unsigned long *) ma->choice_counts;
	unsigned long *rate_starts = (unsigned long *) ma->rate_counts;
	unsigned long *cols = ma->cols;
...
*/
void compute_probabilistic_vector_lrr(SparseMatrix* ma, vector<Real>& v, vector<Real>& u, bool max, bool* locks){
 ...
 */
Real long_run_reward_value_iteration(SparseMatrix* ma, bool max) {
	/* 
 ...
 * @return long-run average vector
 */
Real long_run_reward_ssp_value_iteration(SparseMatrix *ma, SparseMatrixMEC *mecs, vector<Real> lra_mec, bool max) {
	/* 
	 * Define value iteration likewise to time-bounded reachability
	 * with expected time property
	 */
